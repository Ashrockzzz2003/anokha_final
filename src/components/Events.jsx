    ...
    if (events.length) {
      ...
      setFilteredEvents(
        events.filter((event) =>
          ...
          && (event.departmentAbbr.toUpperCase() === departmentAbbr?.toString().toUpperCase() || departmentAbbr === "All" || departmentAbbr === "") && !([89,48,92,51,55,96,98,99,57,7,8,65,111].includes(parseInt(event.eventId)))
        )
      );
    }
